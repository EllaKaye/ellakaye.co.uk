...
Hanukkah of Data uses the same data set for each puzzle, representing a database,
and our task is to find a certain piece of information in it each day.
...
Today was the day I realised that I needed to pay attention to the title of the puzzle: 
the cats are the important thing today.

In retrospect, I suspect that the mention of Noah's sweatshirt was to encourage us to see in the prduct list an item that comes in many colours.
...
Running my original function on the speedrun caused a problem, and this was it.
In the more "challening" dataset, 
there are additions items with colours that *don't* have `"COL"` in their `sku`, and it's one of these that is the common item in this case.
...
and to revel in just how fine (and devious) the storytelling and clue-dropping was.
...
I've expect that the COL/collectible/colour confusion was a deliberate "gotcha" on the part of [Saul Pwanson](https://www.saul.pw), the Hannukah of Data creator, 
...
I didn't want to spoil the surprise for anyone by making the final artwork immediately visible, but for those who want to see it now, they can do so [here](hanukkah-of-data-rug.png).
